...
namespace Rain_World_Drought.PlacedObjects
{
    public class GravityAmplifier : UpdatableAndDeletable, INotifyWhenRoomIsReady, IDrawable
    {
        ...
        public override void Update(bool eu)
        {
            ...
            {
                GravityForce(this.room.waterObject, this.placedObject.pos.x - 400f, this.placedObject.pos.x + 400f, 2f * power);
            }
        }

        public static void GravityForce(Water water, float left, float right, float push)
        {
            int num = water.PreviousSurfacePoint(left);
            int num2 = Custom.IntClamp(water.PreviousSurfacePoint(right) + 1, 0, water.surface.GetLength(0) - 1);
            float center = ((float)num + (float)num2) / 2;
            for (int i = num; i <= num2; i++)
            {
                water.surface[i, 0].height += push * (1f - (Mathf.Abs(center - i) / Mathf.Abs(((float)num - (float)num2) / 2f)));
